...
<?php
	...
	if ($module == 'tbl_brg' and $act == 'uploadImage') {
		...
		if ($terupload) {
			mysqli_query($koneksi, "UPDATE c_imgbrgsedia
			...
			<script type="text/javascript">
				setTimeout(function() {
					swal({
						title: 'SUKSES',
						text: 'DATABASE BERHASIL DIUPDATE',
						type: 'success',
						showConfirmButton: false
					});
				}, 10);
				window.setTimeout(function() {
					window.location.replace('../../appsmedia.php?module=tbl_brg');
				}, 1500);
			</script>
		<?php
			//			header('location:../../media.php?module='.$module);
